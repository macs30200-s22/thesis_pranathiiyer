...
def final_plot():
    Appdf = makeDF(grooms_wanted.wv, appearance)
    # groom seeking adjustments for adjectives only to increase interpretability and remove overlapping words. 
    Appdf.loc['traditional'] = .1
    Appdf.loc['pretty'] = .001
    Appdf.loc['beautiful'] = -.02
    Appdf.loc['graceful'] = -.03
    Appdf.loc['tall'] = -.13
    Appdf.loc['intelligent'] = -.12
    Appdf.loc['brilliant'] = -.108
    Appdf.loc['attractive'] = -.099
    Appdf.loc['progressive'] = -.086
    Appdf.loc['short'] = -.076
    Appdf.loc['slim'] = -.067
    ...
    PlotDimension(ax1, Appdf, 'gender')
